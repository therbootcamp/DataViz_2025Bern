...
2. Load the `sf`, `ggraph` and `tidygraph` packages.
...
5. Add `aes(width = weight)` and `show.legend = FALSE` to the edges function so that the edges are scaled according to the relatedness between quarters, without showing a legend, which in this case is not strictly necessary.  
...
1. Replot the network with out other edge functions, such as `geom_edge_fan()` or `geom_edge_arc()`.

2. Install the `maps` package and then plot a word map with the following code. 

```{r}
map = map('world', plot = FALSE, fill = TRUE)
world = sf::st_as_sf(map)
...
3. Use the following code to rotate the map. Play around with the `lon` and `lat` values. Can you center on Bern? 

```{r}
world_rotated <- sf::st_transform(
  world,
...
4. Try to zoom in on Bern by adding `coord_sf()` and setting the `xlim` and `ylim` arguments. 
